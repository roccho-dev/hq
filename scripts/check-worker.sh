...
assert sum(row["version"] == "worker.decision.v1" for row in normal) == 4, normal
...
assert all(row["version"] == "worker.decision.v1" and row["decision"] == "duplicate" for row in duplicates), duplicates
...
assert len({row.get("decision_id") for row in evidence if row["version"] == "worker.decision.v1"}) == 8
PY
