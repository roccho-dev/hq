...
func TestWSLCCommandCompletesAndLowersSemanticIdentityOnly(t *testing.T) {
	...
	for _, forbidden := range []string{
		"binding_ref", "executable", "materialDigest", "material_digest", "deployment",
		"envctl", `"argv"`, "Program Files", "ProgramFiles", "wslc.exe",
	} {
	...
	ref, ok := world.CommandRef("wslc.version")
	if !ok || ref.CommandID != "wslc.version" || ref.CommandVersion != "1" || ref.Digest == "" {
