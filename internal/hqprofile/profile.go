...
func Load(name, rootOverride string) (Profile, error) {
	...
	path := filepath.Join(filepath.Clean(root), name+".json")
	file, err := os.Open(path)
	if err != nil {
		return Profile{}, fmt.Errorf("open profile %q: %w", name, err)
	}
	defer file.Close()
	profile, err := Decode(file)
	if err != nil {
		return Profile{}, fmt.Errorf("decode profile %q: %w", name, err)
	}
	if profile.Name != name {
		return Profile{}, fmt.Errorf("profile name mismatch: requested %q, got %q", name, profile.Name)
	}
...
func (p Profile) Validate() error {
	...
	for field, value := range map[string]string{
		"world_path": p.WorldPath, "accepted_path": p.AcceptedPath,
		"workspace_root": p.WorkspaceRoot, "events_path": p.EventsPath,
		"capabilities_path": p.CapabilitiesPath,
	} {
		if strings.TrimSpace(value) == "" || !filepath.IsAbs(value) {
			return fmt.Errorf("%s must be a non-empty absolute path", field)
		}
	}
	if p.PollIntervalMS < 20 || p.PollIntervalMS > 60_000 {
		return errors.New("poll_interval_ms must be between 20 and 60000")
	}
	if p.HealthTimeoutMS < p.PollIntervalMS*2 || p.HealthTimeoutMS > 300_000 {
...
func (p Profile) PollInterval() time.Duration { return time.Duration(p.PollIntervalMS) * time.Millisecond }
func (p Profile) HealthTimeout() time.Duration { return time.Duration(p.HealthTimeoutMS) * time.Millisecond }
