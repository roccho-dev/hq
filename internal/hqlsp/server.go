...
type document struct { Text string; Version int }
...
func New(profile hqprofile.Profile) (*Server, error) {
	...
	if err != nil { return nil, fmt.Errorf("open profile world: %w", err) }
	defer file.Close()
	world, err := hq.LoadSchemaJSONL(file)
	if err != nil { return nil, fmt.Errorf("load profile world: %w", err) }
...
func (s *Server) Serve(r io.Reader, w io.Writer) error {
	...
	for {
		...
		if msg.Method == "exit" { return nil }
		if err := s.handle(w, msg); err != nil { return err }
	}
}
...
func (s *Server) handle(w io.Writer, msg message) error {
	...
	case "shutdown": return writeMessage(w, message{JSONRPC: "2.0", ID: msg.ID, Result: nil})
	case "textDocument/didOpen":
		var p struct { TextDocument struct { URI string `json:"uri"`; Text string `json:"text"`; Version int `json:"version"` } `json:"textDocument"` }
	...
	case "textDocument/didChange":
		var p struct {
			TextDocument struct { URI string `json:"uri"`; Version int `json:"version"` } `json:"textDocument"`
			ContentChanges []struct { Text string `json:"text"` } `json:"contentChanges"`
		}
		if err := json.Unmarshal(msg.Params, &p); err != nil { return err }
		if len(p.ContentChanges) == 0 { return nil }
		s.documents[p.TextDocument.URI] = document{Text: p.ContentChanges[len(p.ContentChanges)-1].Text, Version: p.TextDocument.Version}
		return s.publishDiagnostics(w, p.TextDocument.URI)
	case "textDocument/completion": return s.complete(w, msg)
	case "textDocument/codeAction": return s.codeAction(w, msg)
	case "workspace/executeCommand": return s.executeCommand(w, msg)
	default:
		if len(msg.ID) != 0 { return writeError(w, msg.ID, -32601, "method not found") }
		return nil
	}
}
...
func (s *Server) complete(w io.Writer, msg message) error {
	var p struct {
		...
		Position struct { Line int `json:"line"`; Character int `json:"character"` } `json:"position"`
	}
	if err := json.Unmarshal(msg.Params, &p); err != nil { return writeError(w, msg.ID, -32602, err.Error()) }
	doc, ok := s.documents[p.TextDocument.URI]
	if !ok { return writeError(w, msg.ID, -32602, "document is not open") }
	cursor, err := byteOffset(doc.Text, p.Position.Line, p.Position.Character)
	if err != nil { return writeError(w, msg.ID, -32602, err.Error()) }
	suggestions := hq.Complete(doc.Text, cursor, s.world)
	items := make([]map[string]any, 0, len(suggestions))
	for _, suggestion := range suggestions {
		items = append(items, map[string]any{
			"label": suggestion.Label, "detail": suggestion.Detail, "kind": 14,
			"insertText": suggestion.InsertText, "insertTextFormat": 1,
...
func (s *Server) codeAction(w io.Writer, msg message) error {
	...
	if !ok { return writeMessage(w, message{JSONRPC: "2.0", ID: msg.ID, Result: []any{}}) }
	actions := []map[string]any{{
		"title": "HQ Submit", "kind": "quickfix",
		"command": map[string]any{"title": "HQ Submit", "command": "hq.submit", "arguments": []any{map[string]any{"uri": uri, "version": doc.Version}}},
	}}
...
func (s *Server) executeCommand(w io.Writer, msg message) error {
	var p struct {
		...
		Arguments []struct { URI string `json:"uri"`; Version int `json:"version"` } `json:"arguments"`
	}
	if err := json.Unmarshal(msg.Params, &p); err != nil { return writeError(w, msg.ID, -32602, err.Error()) }
	if p.Command != "hq.submit" || len(p.Arguments) != 1 { return writeError(w, msg.ID, -32602, "hq.submit requires one document argument") }
	arg := p.Arguments[0]
	doc, ok := s.documents[arg.URI]
	if !ok || doc.Version != arg.Version { return writeError(w, msg.ID, -32602, "document version is missing or stale") }
	draft := hq.CompileLine(doc.Text, s.world)
	encoded, err := json.Marshal(draft)
	if err != nil { return writeError(w, msg.ID, -32603, err.Error()) }
	if err := appendAndSync(s.profile.AcceptedPath, draft); err != nil { return writeError(w, msg.ID, -32603, err.Error()) }
	digest := sha256.Sum256(encoded)
	result := map[string]any{
		"kind": SubmitResultKind, "status": "queued", "queueKind": draft.Kind,
		"queueId": "accepted-sha256-" + hex.EncodeToString(digest[:8]), "deploymentId": s.profile.DeploymentID,
	}
...
func (s *Server) publishDiagnostics(w io.Writer, uri string) error {
	...
	if trimmed != "" && !json.Valid([]byte(trimmed)) {
		diagnostics = append(diagnostics, map[string]any{
			"range": map[string]any{"start": map[string]int{"line": 0, "character": 0}, "end": map[string]int{"line": 0, "character": utf8.RuneCountInString(doc.Text)}},
			"severity": 1, "source": "hq", "code": "invalid-json", "message": "buffer is not one complete JSON object",
		})
	}
...
func appendAndSync(path string, draft hq.CompileDraft) error {
	...
	if err != nil { return fmt.Errorf("open accepted queue: %w", err) }
	writer := hq.QueueWriter{W: file}
	if err := writer.Append(draft); err != nil { _ = file.Close(); return err }
	if err := file.Sync(); err != nil { _ = file.Close(); return err }
	return file.Close()
}

func uriFromTextDocument(raw json.RawMessage) (string, error) {
	var p struct { TextDocument struct { URI string `json:"uri"` } `json:"textDocument"` }
	if err := json.Unmarshal(raw, &p); err != nil { return "", err }
	if p.TextDocument.URI == "" { return "", errors.New("textDocument.uri is required") }
	return p.TextDocument.URI, nil
}

func byteOffset(text string, line, character int) (int, error) {
	if line < 0 || character < 0 { return 0, errors.New("position cannot be negative") }
	start := 0
	for current := 0; current < line; current++ {
		index := strings.IndexByte(text[start:], '\n')
		if index < 0 { return 0, errors.New("position line is outside document") }
		start += index + 1
	}
	end := strings.IndexByte(text[start:], '\n')
	if end < 0 { end = len(text) - start }
	...
	for byteIndex < len(lineText) {
		...
		units += width
		byteIndex += size
		if units == character { return start + byteIndex, nil }
	}
	if units == character { return start + byteIndex, nil }
