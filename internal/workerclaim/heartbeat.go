...
type Heartbeat struct {
	Kind string `json:"kind"`; ClaimID string `json:"claim_id"`; WorkerID string `json:"worker_id"`; Workspace string `json:"workspace"`
	DeploymentID string `json:"deployment_id"`; Profile string `json:"profile"`; State string `json:"state"`; ObservedAt time.Time `json:"observed_at"`
}
...
func (c *Claim) WriteHeartbeat(deploymentID, profile, state string, now time.Time) (Heartbeat, error) {
	...
	current, err := readOwner(c.path); if err != nil { return Heartbeat{}, err }
	...
	file, err := os.OpenFile(path, os.O_CREATE|os.O_TRUNC|os.O_WRONLY, 0o600); if err != nil { return Heartbeat{}, err }
	encoder := json.NewEncoder(file); encoder.SetEscapeHTML(false)
	writeErr := encoder.Encode(record); if writeErr == nil { writeErr = file.Sync() }; closeErr := file.Close()
	if writeErr != nil { return Heartbeat{}, writeErr }; if closeErr != nil { return Heartbeat{}, closeErr }
	return record, nil
}
...
func ReadHeartbeat(projectRoot string) (Heartbeat, error) {
	...
	layout, err := workersafety.NewLayout(root); if err != nil { return Heartbeat{}, err }
...
func RemoveHeartbeat(projectRoot string) error {
	...
	layout, err := workersafety.NewLayout(root); if err != nil { return err }
	err = os.Remove(filepath.Join(layout.ArtifactRoot, "worker", heartbeatName)); if os.IsNotExist(err) { return nil }; return err
}
...
func RecoverManaged(projectRoot, expectedClaimID, reason string, now time.Time, staleAfter time.Duration) (RecoveryReceipt, error) {
	_, fresh, err := HeartbeatFresh(projectRoot, expectedClaimID, now, staleAfter); if err != nil { return RecoveryReceipt{}, err }
	if fresh { return RecoveryReceipt{}, errors.New("managed worker heartbeat is fresh; refusing recovery") }
	receipt, err := Recover(projectRoot, expectedClaimID, reason, now, staleAfter); if err != nil { return RecoveryReceipt{}, err }
	_ = RemoveHeartbeat(projectRoot); return receipt, nil
}
