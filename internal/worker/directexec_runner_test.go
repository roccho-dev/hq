...
func directexecRow(t *testing.T, id, cwd string, argv []string) ReadRow {
	t.Helper()
...
func TestRunnerDefaultRegistryExecutesApprovedDirectProcessOnce(t *testing.T) {
	helper := buildDirectexecHelper(t)
	workspace := t.TempDir()
	sentinel := filepath.Join(workspace, "sentinel.txt")
	row := directexecRow(t, "ins-direct-runner-001", workspace, []string{
	...
	runner := NewRunner(workspace, emptyRegistry, approvedRunnerFixture(t, row.Instruction))
	sink := &memoryAppender{}
	...
	secondSink := &memoryAppender{}
	second, unsuccessful, err := runner.Process(context.Background(), []ReadRow{row}, prior, false, secondSink)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunnerMissingApprovalDoesNotStartDirectProcess(t *testing.T) {
	...
	workspace := t.TempDir()
	sentinel := filepath.Join(workspace, "sentinel.txt")
	row := directexecRow(t, "ins-direct-blocked-001", workspace, []string{helper, "--sentinel", sentinel})
	runner := NewRunner(workspace, nil, EmptyApprovalStore())
...
func TestRunnerMapsDirectProcessNonZeroExit(t *testing.T) {
	helper := buildDirectexecHelper(t)
	workspace := t.TempDir()
	row := directexecRow(t, "ins-direct-exit-001", workspace, []string{helper, "--exit-code", "9"})
	runner := NewRunner(workspace, nil, approvedRunnerFixture(t, row.Instruction))
	emitted, unsuccessful, err := runner.Process(context.Background(), []ReadRow{row}, LogData{}, false, &memoryAppender{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunnerMapsDirectProcessDeadlineWithPartialOutput(t *testing.T) {
	helper := buildDirectexecHelper(t)
	workspace := t.TempDir()
	row := directexecRow(t, "ins-direct-timeout-001", workspace, []string{
...
func TestRunnerMapsExplicitDirectProcessCancellation(t *testing.T) {
	helper := buildDirectexecHelper(t)
	workspace := t.TempDir()
	row := directexecRow(t, "ins-direct-cancel-001", workspace, []string{helper, "--sleep", "5s"})
	runner := NewRunner(workspace, nil, approvedRunnerFixture(t, row.Instruction))
