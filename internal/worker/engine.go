...
func (e Engine) Plan(row ReadRow, validation []Diagnostic) PlanRow {
	...
	if plan.Policy.Allowed {
		plan.Decision = DecisionAccepted
	}
	return plan
}
...
func (e Engine) DryRun(rows []ReadRow, w io.Writer) (blocked int, err error) {
	...
	for i, row := range rows {
		...
		if plan.Decision != DecisionAccepted {
			blocked++
		}
...
// EvaluateNormal records worker decisions and result.v1 rows but never calls a
...
func (e Engine) EvaluateNormal(rows []ReadRow, prior LogData, replay bool) []LogEntry {
	...
	entries := make([]LogEntry, 0, len(rows)*3)
	for i, row := range rows {
		at := now().UTC()
		if len(validation[i]) != 0 {
			entries = append(entries, DecisionEntry(e.makeDecision(row, "", DecisionBlocked, validation[i][0].Code, validation[i][0].Message, validation[i], nil, at)))
			continue
		}
		if priorRun, exists := priorRuns[row.Instruction.ID]; exists && !replay {
			if priorRun.Status == StatusQueued {
				...
				entries = append(entries, DecisionEntry(e.makeDecision(row, priorRun.RunID, DecisionBlocked, decisionCode(decision), decisionMessage(decision), nil, &decision, at)))
				entries = append(entries, ResultEntry(makeBlockedResult(priorRun.RunID, row.Instruction, priorRun.NextSeq, decision, at.Add(time.Nanosecond))))
				continue
			}
			entries = append(entries, DecisionEntry(e.makeDecision(row, priorRun.RunID, DecisionDuplicate, "duplicate_instruction", "instruction already has durable run evidence; use --replay to create a new run", nil, nil, at)))
			continue
		}

		runID := makeRunID(row, at)
		decision := e.Policy.Evaluate(row.Instruction)
		entries = append(entries, DecisionEntry(e.makeDecision(row, runID, DecisionBlocked, decisionCode(decision), decisionMessage(decision), nil, &decision, at)))
		entries = append(entries, ResultEntry(ResultRow{
			...
			Kind:          ResultAccepted,
			Seq:           0,
			RecordedAt:    at.Add(time.Nanosecond),
		}))
		entries = append(entries, ResultEntry(makeBlockedResult(runID, row.Instruction, 1, decision, at.Add(2*time.Nanosecond))))
	}
	return entries
}

func (e Engine) makeDecision(row ReadRow, runID, kind, code, message string, diagnostics []Diagnostic, policy *PolicyDecision, at time.Time) DecisionRow {
	return DecisionRow{
		DecisionID:      makeDecisionID(row, at),
		Version:         DecisionVersionV1,
		Decision:        kind,
		RecordedAt:      at,
		Source:          row.Source,
		InstructionID:   row.Instruction.ID,
...
func decisionCode(decision PolicyDecision) string {
	if decision.Allowed {
...
func decisionMessage(decision PolicyDecision) string {
	if decision.Allowed {
		return "validated instruction has no installed concrete adapter"
	}
...
func makeDecisionID(row ReadRow, at time.Time) string {
	seed := fmt.Sprintf("decision\x00%s\x00%d\x00%s\x00%d", row.Source.Path, row.Source.Line, row.Instruction.ID, at.UnixNano())
	h := sha256.Sum256([]byte(seed))
	return "decision-" + hex.EncodeToString(h[:8])
}
