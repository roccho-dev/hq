...
func runInteractive(world *hq.JsonlWorld, queuePath string, banner bool) error {
	rl := readline.NewShell(inputrc.WithName("hq-reflective"))
	...
	// For this POC, Tab opens the visible candidate list. This makes the expected
	// Cursor-like UX inspectable: candidate label + detail + meaning/compile target.
	...
	if banner {
		fmt.Println("hq-reflective: vi=ON Tab=candidates autocomplete=ON Ctrl-T=compileDraft Enter=queue")
