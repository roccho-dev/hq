...
> Canonical `target=sh` means direct explicit-path argv execution, not shell language. The common worker owns every safety and durable lifecycle decision; the adapter owns only one direct child and transient output.
